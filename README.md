...
npm ci
cp .env.example .env
# Fill in DATABASE_URL, BETTER_AUTH_SECRET, BETTER_AUTH_URL, HR_INITIAL_PASSWORD, BLOB_STORE_ID.
npm run db:migrate
...
Connect a public Vercel Blob store to the project and set `BLOB_STORE_ID`. Vercel supplies short-lived project OIDC credentials automatically in production; local upload scripts can use the development OIDC credential generated by `vercel link`. A store-scoped `BLOB_READ_WRITE_TOKEN` is an optional fallback outside Vercel. `scripts/upload-seed-portraits.ts` downloads the documented portrait sources, uploads them to `employee-portraits/seed/`, writes the public URL manifest, and migrates existing demo photo paths. Set `PORTRAIT_SOURCE_DIR` to use already-downloaded originals. Binary images are not committed to GitHub or stored in Postgres.
...
- `BLOB_STORE_ID`: the connected Vercel Blob store ID. Vercel automatically provides rotating OIDC authentication; a static Blob token is not required.
